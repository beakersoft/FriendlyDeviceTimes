...
function renderTimetable(schedule) {
  ...
  for (const day of DAYS) {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      cell.setAttribute('aria-label', `${day} ${hourToLabel(hour)} ${allowed ? 'allowed' : 'blocked'}`);
...
function renderSummary(schedule) {
  ...
  for (const day of DAYS) {
    ...
    if (daySchedule.dailyLimitMinutes !== null) {
      parts.push(`Daily limit: ${formatMinutes(daySchedule.dailyLimitMinutes)}`);
    }
...
function render(data) {
  ...
  if (data.child?.deviceCount) {
    parts.push(`${data.child.deviceCount} device${data.child.deviceCount === 1 ? '' : 's'}`);
